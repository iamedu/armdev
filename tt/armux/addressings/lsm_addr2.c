...
void lsm_addr2(ARMProc *proc, UWord instruction, void *result) {
	ARMAddrLSMReturn *res = result;
        Word Rn, register_list, W;

#ifdef DEBUG
        printf("Ejecutaste un LSM addr2\n");
#endif
	Rn = get_bits(instruction, 16, 4);
        register_list = get_bits(instruction, 0, 16);
        W = get_bits(instruction, 21, 1);
        res->start_address = *proc->r[Rn] + 4;
        res->end_address = *proc->r[Rn] + (count32(register_list) * 4);
