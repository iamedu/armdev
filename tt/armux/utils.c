...
Word cond(ARMProc *proc, UWord inst){
	Word something = get_bits(inst, 28, 4);
	Word n,z,c,v;
	switch(something){
		case 0:
			z = get_bits(*proc->cpsr,30,1);
			return z == 1;
		break;
		case 1:
			z = get_bits(*proc->cpsr,30,1);	
		...
		break;
		case 13:
			n = get_status(proc, status_n);
			z = get_status(proc, status_z);
			v = get_status(proc, status_v);
			return(z==1 || n!=v);
		break;
		case 14:
			return 1; 
...
Word OverflowFrom(Word lastValue, Word newValue){
	if(get_bits(lastValue,31,1) == 0 && get_bits(newValue,31,1) == 1)
            return 1;
        else
            return 0;
}
